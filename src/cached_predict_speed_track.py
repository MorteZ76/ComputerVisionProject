...
PRED_EXPAND = 1.10  # enlarge predicted boxes while lost
...
# --- Lost handling ---
LOST_KEEP = 35                 # drop track after >20 lost frames
DISABLE_LOW_WHEN_LOST = True   # lost tracks never match low-conf dets
CENTER_DIST_GATE = 60.0        # px, “kinda close” for revival via high-conf match
...
class ByteTrackLike:
    def __init__(self, iou_gate=IOU_GATE, min_hits=MIN_HITS):
        ...
        self.vel_hist = {}  # id -> deque of (vx, vy) from high-conf matches


    # ---------- helpers ----------
    def _clear_future_for(self, tid, from_frame):
        # remove all scheduled predictions for tid from from_frame onward
    ...
    def _get_blended_velocity(self, t):
        ...
        if hist and len(hist) > 0:
            ...
            vx = 0.7 * vx_mean + 0.3 * vx_last
            vy = 0.7 * vy_mean + 0.3 * vy_last
        else:
            if HAS_FILTERPY:
    ...
    def _get_mean_velocity(self, t):
        ...
        if hist and len(hist) > 0:
            ...
            return vx, vy
        if HAS_FILTERPY:
            return float(t.kf.x[4,0]), float(t.kf.x[5,0])
        return 0.0, 0.0


    def _schedule_future(self, t: 'Track', cur_frame: int, horizon: int = LOST_KEEP):
        # base box from current state
        if HAS_FILTERPY:
            cx, cy, w, h = [float(v) for v in t.kf.x[:4, 0]]
        else:
            cx, cy, w, h = [float(v) for v in t.state]

        # velocity = blended mean+last from history (fallback to KF)
        # vx, vy = self._get_blended_velocity(t)
        vx, vy = self._get_mean_velocity(t)

        # enlarge predicted box a bit
        w *= PRED_EXPAND
        h *= PRED_EXPAND
        ...
        for k in range(1, horizon + 1):
            cxk = cx + k * vx
            cyk = cy + k * vy
            vbox = cxcywh_to_xyxy(np.array([cxk, cyk, max(1.0, w), max(1.0, h)], np.float32))
    ...
    def _match(self, tracks, dets):
        ...
        # dynamic center gate + Mahalanobis (when KF available)
    ...
    def _revive_from_future(self, dets_high, frame_idx):
        ...
        # predicted boxes and their track ids
        ...
        for r in range(len(pboxes)):
            ...
            gate = max(CENTER_DIST_GATE, 0.50 * diag)  # scale by predicted size
        ...
        for ri, ci in zip(r, c):
            ...
            # apply revival update to the corresponding track
            # find track with id == tid
            for t in self.tracks:
                if t.id == tid:
                    ...
                    if dqv is None:
                        dqv = deque(maxlen=KINEMA_WINDOW); self.vel_hist[t.id] = dqv
                    dqv.append((vx, vy))

                    # maintain area history for size sanity
                    a = _area_xyxy(d["bbox"])
        ...
        # clear consumed predictions for these revived tids from this and future frames
    ...
    def update(self, detections, frame_idx, frame_shape=None):
        ...
        if active_trs and high:
            ...
            for ai, dj in mA:
                ...
                dq.append(a)
                # velocity hist (for true-mean predictions)
                if HAS_FILTERPY:
                    vx = float(t.kf.x[4,0]); vy = float(t.kf.x[5,0])
                else:
                    vx = vy = 0.0
        ...
        if len(high) > 0 and len(used_high) > 0:
            idx_map = [i for i in range(len(high)) if i not in used_high]  # map left-list -> original
        ...
        if used_det_idx:
            # mark those original high indices as used
        ...
        if low:
            # which active tracks are still unmatched?
        ...
        for i, t in enumerate(self.tracks):
            if i not in matched_global:
                ...
                t.mark_missed()
                if prev_tsu == 0:
                    self._schedule_future(t, frame_idx, horizon=LOST_KEEP)
...
def main():
    ...
    while True:
        ...
        for t in tracks:
            ...
            # log for HOTA: include predicted boxes while lost
            ...
            if len(hist) > 0:
                ...
                speed_val = math.hypot(vx_mean, vy_mean)
                # angle_val = math.degrees(math.atan2(vy_blend, vx_blend))
                angle_val = math.degrees(math.atan2(vy_mean, vx_mean))
            else:
